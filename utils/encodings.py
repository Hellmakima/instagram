...
EXCLUDED_FOLDERS = ["__pycache__", "venv", ".git", "node_modules", ".next"]
...
def is_ascii_file(file_path):
    encodings_to_try = [
        ...
        "utf-16",
        "latin1",
    ]  # latin1 handles anything without error
...
def check_ascii_in_folder(root_folder):
    ...
    for dirpath, _, filenames in os.walk(root_folder):
        ...
        for file in filenames:
            ...
            if any(
                file.endswith(excluded_extention)
                for excluded_extention in EXCLUDED_EXTENTIONS
            ):
                continue
            if any(
                file.startswith(excluded_prefix) for excluded_prefix in ["mnbvbwrj"]
            ):
                continue
