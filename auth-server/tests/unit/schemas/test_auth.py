...
def test_username_validator_valid():
    ...
    for u in valid:
        obj = auth.UserCreate(username=u, email="test@gmail.com", password="Aa1!password")
...
def test_email_validator_valid():
    obj = auth.UserCreate(username="user1", email="test@gmail.com", password="Aa1!password")
    assert obj.email == "test@gmail.com"

def test_email_validator_invalid():
    with pytest.raises(ValidationError):
        auth.UserCreate(username="user1", email="test@yahoo.com", password="Aa1!password")

def test_password_validator_valid():
    obj = auth.UserCreate(username="user1", email="test@gmail.com", password="Abcdef1!23")
