...
@pytest.mark.asyncio
@patch(
    "app.repositories.refresh_token.datetime",
    MagicMock(now=MagicMock(return_value=MOCKED_NOW)),
)
async def test_find_by_token_calls_collection_correctly(
    mock_refresh_token_db, mock_refresh_token_collection
):
    ...
    mock_refresh_token_collection.find_one = AsyncMock(
        return_value={"_id": "abc", "user_id": "user123"}
    )
...
async def test_delete_by_token_calls_delete_one(
    ...
    mock_refresh_token_collection.delete_one.assert_awaited_once_with(
        {"refresh_token": token}
    )
...
async def test_find_by_token_rejects_revoked_token(
    mock_refresh_token_db, mock_refresh_token_collection
):
    repo = refresh_token_repo.RefreshToken(mock_refresh_token_db)
    token = "token123"
...
async def test_find_by_token_rejects_expired_token(
    mock_refresh_token_db, mock_refresh_token_collection
):
    repo = refresh_token_repo.RefreshToken(mock_refresh_token_db)
    token = "expired_token"
