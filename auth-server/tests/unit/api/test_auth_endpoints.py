...
def test_register_calls_service(app):
    ...
    with patch('app.api.api_v1.endpoints.auth.router.create_user_service', new=mock_create):
        ...
        try:
            with TestClient(app) as client:
                ...
                payload = {"username": "user1", "email": "user1@gmail.com", "password": "Abcdef1!23"}
                resp = client.post('/v1/auth/register', json=payload, headers={'X-CSRF-Token': csrf})
...
def test_login_sets_cookies(app):
    ...
    with patch('app.api.api_v1.endpoints.auth.router.login_user_service', new=mock_login):
...
def test_logout_calls_service_and_deletes_cookies(app):
    ...
    with patch('app.api.api_v1.endpoints.auth.router.logout_user_service', new=mock_logout):
...
def test_refresh_token_rotates_tokens(app):
    ...
    with patch('app.api.api_v1.endpoints.auth.router.refresh_access_token_service', new=mock_refresh):
        ...
        try:
            with TestClient(app) as client:
                ...
                resp = client.post('/v1/auth/refresh_token', headers={'X-CSRF-Token': csrf})
                assert resp.status_code == 200
