...
def test_send_verification_email_endpoint_calls_service(app):
    ...
    with patch('app.api.api_v1.endpoints.email.router.send_verification_email_service', new=mock_send):
        ...
        try:
            with TestClient(app) as client:
                ...
                resp = client.get(f'/v1/email/send-verification-email?user_id=testid&email=test@gmail.com', headers={'X-CSRF-Token': csrf})
...
def test_verify_email_endpoint_calls_service(app):
    ...
    with patch('app.api.api_v1.endpoints.email.router.verify_email_token_service', new=mock_verify):
        from app.api.dependencies.db_deps import get_user_repo
        original = app.dependency_overrides.get(get_user_repo)
        app.dependency_overrides[get_user_repo] = lambda db=None: object()
        try:
