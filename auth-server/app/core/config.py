...
class Settings(BaseSettings):
    ...
    REFRESH_TOKEN_COLLECTION: str = Field(
        ..., description="Refresh token collection name"
    )
    ...
    # Refresh Token (HS256)
    REFRESH_TOKEN_JWT_SECRET_KEY: str = Field(
        ..., description="Secret key for refresh tokens"
    )
    REFRESH_TOKEN_JWT_ALGORITHM: str = Field(
        ..., description="Algorithm for refresh tokens"
    )
    REFRESH_TOKEN_EXPIRE_DAYS: int = Field(..., description="Refresh token expire days")
    REFRESH_TOKEN_EXPIRE_MINUTES: int = Field(
        1, description="Refresh token expire minutes"
    )

    # Access Token (RS256)
    ACCESS_TOKEN_PUBLIC_JWT_SECRET_KEY: str = Field(
        ..., description="Public key for access tokens"
    )
    ACCESS_TOKEN_PRIVATE_JWT_SECRET_KEY: str = Field(
        ..., description="Private key for access tokens"
    )
    ACCESS_TOKEN_JWT_ALGORITHM: str = Field(
        ..., description="Algorithm for access tokens"
    )
    ...
    # Email Service
    EMAIL_VERIFICATION_TOKEN_JWT_ALGORITHM: str = Field(
        ..., description="Algorithm for email verification token"
    )
    EMAIL_VERIFICATION_TOKEN_SECRET_KEY: str = Field(
        ..., description="Key for email verification token"
    )
    EMAIL_VERIFICATION_TOKEN_EXPIRE_MINUTES: int = Field(
        ..., description="Expire minutes for email verification token"
    )
